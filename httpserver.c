...
int transferfile(int clientfd, FILE *fp, int type, int rangestart, int rangetotal);
	...
	while(1){
		...
		*thisParam = new_fd;
		...
		if(pthread_create(&threadid, NULL, threadfunction, thisParam) != 0){
			...
			/*After finishing the thread function, the resource will automatically freed*/
...
int getrequest(char *requestbuf, struct HttpRequest *httprequest){
	...
	/*If the first line is less than 3 elements, it means the request have some problem.*/
	...
	if(strcmp(httprequest->method, "GET") == 0){
		...
		printf("Path: %s\nPrefix: %s\n", httprequest->path, httprequest->prefix);
...
void getcommon(int clientfd, struct HttpRequest *httprequest){
	...
	/*If file exists*/
		...
		transferfile(clientfd, fp, httprequest->rangeflag, httprequest->rangestart, httprequest->rangetotal);
	...
	switch(code){
		case 200:
			sprintf(buffer,
					...
					"Content-Length: %ld\r\n"
					"Connection: close\r\n"
					"\r\n", getcontenttype(httprequest->prefix), httprequest->rangetotal);
			break;

		case 404:
			strcpy(content, "<html><head><title>404 Not Found</title></head><body><h1>404 Not Found</h1>File Not Found.</body></html>");
			sprintf(buffer,
					...
					"Connection: close\r\n"
					"\r\n"
					"%s", "text/plain", strlen(content), content);
			break;
...
int transferfile(int clientfd, FILE *fp, int type, int rangestart, int totallength){
	if(type == 1){
		fseek(fp, rangestart, 0);
	}

	int sendnum = 0;
	int segment = 1024;

	/*feof(fp)-->If the data has not been read completely, it will return zero*/
